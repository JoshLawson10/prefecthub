...
export function TasksView({ tasks: initialTasks, scopedToEvent = false }: TasksViewProps) {
  ...
  const toggleDone = useCallback(async (task: Task) => {
    const current = statusOverrides[task.id] ?? task.status;
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              {!scopedToEvent && <TableHead className="hidden sm:table-cell">Event</TableHead>}
          ...
          <TableBody>
            {paginated.length === 0 ? (
              ...
              return (
                <TableRow key={task.id} className={cn("group transition-colors", isDone && "opacity-60")}>
                  ...
                  <TableCell className="max-w-xs">
                    <p className={cn("font-medium truncate", isDone && "line-through text-muted-foreground")}>{task.title}</p>
                    {task.description && <p className="text-xs text-muted-foreground truncate mt-0.5">{task.description}</p>}
                  </TableCell>
                  {!scopedToEvent && (
                    <TableCell className="hidden sm:table-cell">
                      <Link href={`/events/${task.event_id}/tasks`} className="text-xs text-muted-foreground hover:text-foreground transition-colors truncate max-w-36 block">
                        {task.event_title}
                      </Link>
                    </TableCell>
                  )}
                  <TableCell>
                    <Avatar size="sm"><AvatarFallback>{task.assignee_initials ?? "?"}</AvatarFallback></Avatar>
                  </TableCell>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-border px-4 py-3">
            ...
            <div className="flex items-center gap-1">
              <Button variant="outline" size="icon-sm" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={safePage === 1} aria-label="Previous page">
                <ChevronLeftIcon className="size-3.5" />
              </Button>
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                <Button key={p} variant={p === safePage ? "default" : "outline"} size="icon-sm" onClick={() => setPage(p)} aria-label={`Page ${p}`} aria-current={p === safePage ? "page" : undefined}>
                  {p}
                </Button>
              ))}
              <Button variant="outline" size="icon-sm" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={safePage === totalPages} aria-label="Next page">
