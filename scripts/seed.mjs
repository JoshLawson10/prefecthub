...
console.log('  Workspace    : ' + SEED_WORKSPACE + ' (' + SEED_SCHOOL + ', ' + SEED_YEAR + ')');
console.log('');
...
await step('Create auth user', async () => {
  ...
  if (error) {
    if (error.message.includes('already been registered') ||
      ...
      if (!existing) throw new Error('User exists but could not be found: ' + error.message);
...
await step('Create workspace', async () => {
  ...
  const { data, error } = await admin
    .from('workspaces')
    .insert({ name: SEED_WORKSPACE, school: SEED_SCHOOL, year: SEED_YEAR, created_by: authUserId })
    .select('id')
    .single();
...
await step('Create sample event', async () => {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      description: 'Annual graduation ceremony for the Year 12 cohort. All prefects are required to assist with guest management and venue setup.',
  ...
  const { error } = await admin.from('tasks').insert([
    {
      ...
      description: 'Email the admin office to confirm the hall booking and AV setup requirements.',
      priority: 'high',
      status: 'todo',
    ...
    },
    {
      ...
      description: 'Create a seating plan using the RSVP data once registration closes.',
      priority: 'medium',
      status: 'todo',
    ...
    },
    {
      ...
      description: 'Run a 15-minute briefing with all rostered prefects on arrival procedures.',
      priority: 'medium',
      status: 'todo',
...
await step('Create welcome notification', async () => {
  ...
  const { error } = await admin.from('notifications').insert({
    ...
    description: 'Your workspace is set up. Start by exploring the dashboard or creating your first event.',
